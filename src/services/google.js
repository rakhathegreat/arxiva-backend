	...
	try {
		...
		if (searchFolder.data.files && searchFolder.data.files.length > 0) {
			...
			const created = await drive.files.create({
				requestBody: { name: 'BAST ARXIVA', mimeType: 'application/vnd.google-apps.folder' },
				fields: 'id',
			});
		...
		const uploaded = await drive.files.create({
			...
			media: { mimeType: 'application/pdf', body: fs.createReadStream(absoluteFilePath) },
			fields: 'id, webViewLink',
		});

		const driveFileId = uploaded.data.id;
		const driveViewUrl = uploaded.data.webViewLink || `https://drive.google.com/file/d/${driveFileId}/view`;
