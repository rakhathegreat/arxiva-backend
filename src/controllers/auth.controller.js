...
function createPendingState(userId) {
    ...
    pendingOAuthStates.set(state, { userId, expiresAt: now + PENDING_STATE_TTL_MS });
    return state;
}
...
async function exchangeAndConnect(code, adminUserId) {
    ...
    const currentUser = await prisma.user.findUnique({ where: { id: adminUserId } });
...
<script>setTimeout(function(){ window.close(); }, 1500);</script>
    ...
    if (error) {
        ...
        return page('Koneksi dibatalkan', 'Otorisasi Google tidak diselesaikan.', false);
    }
    ...
    try {
        ...
        return page('Akun Google terhubung', `${googleEmail} — silakan kembali ke aplikasi Taslim.`, true);
    } catch (err) {
        console.error('Error in Google callback:', err);
        return page('Gagal menghubungkan', err.message || 'Terjadi kesalahan saat menghubungkan akun Google.', false);
    }
};
